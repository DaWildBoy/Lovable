/*
  # Fix Booking Likelihood for Near-Range Prices

  1. Changes
    - Prices within $10 or 2% of recommended LOW now get 80-84% likelihood
    - Removed decimal points - all scores are whole numbers
    - More forgiving for prices that are very close to recommended range
  
  2. New Scoring Logic
    - At or above HIGH = 100%
    - In recommended range (LOW to HIGH) = 85-99%
    - Within $10 or 2% below LOW = 80-84% (good score for near-miss)
    - Below that threshold but above 50% of LOW = max 39%
    - Below 50% of LOW = max 20%
    - $0 = 0%
*/
...
BEGIN
  ...
  -- Calculate near-threshold (within $10 or 2% of LOW, whichever is larger)
  ...
  -- Check if price is at or above recommended HIGH
  ...
  -- Check if price is in recommended range (LOW to HIGH)
  ...
  -- NEW: Check if price is very close to LOW (within $10 or 2%)
  IF p_customer_offer_ttd >= (recommended_low - near_threshold) THEN
    -- Very close to recommended range: 80-84%
    -- Scale based on how close to LOW
    range_position := (p_customer_offer_ttd - (recommended_low - near_threshold)) / near_threshold;
    range_position := LEAST(GREATEST(range_position, 0), 1);
    -- Scale: 80 + (range_position * 4) gives us 80 to 84
  ...
  -- Calculate price ratio for prices well below LOW
  price_ratio := p_customer_offer_ttd / p_recommended_mid_ttd;
  
  -- Price scoring for prices well below LOW
