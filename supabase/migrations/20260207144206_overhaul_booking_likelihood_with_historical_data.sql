/*
  # Overhaul Booking Likelihood with Historical Job Data

  ## Summary
  Redesigns the booking likelihood calculation to incorporate historical
  completed job data, providing more accurate predictions based on real
  market behavior.

  ## Changes
  1. **Historical price lookup**: Queries completed/delivered jobs with
     similar distance range and cargo size to find average accepted prices
  2. **Historical factor blending**: When historical data exists, the
     recommended price is blended with historical averages for more
     accurate scoring
  3. **Cargo count awareness**: More cargo items slightly reduce likelihood
     (harder for couriers to handle)
  4. **Multi-stop awareness**: More stops slightly reduce likelihood
  5. **Improved label messages** with more granular thresholds
  ...
  - Looks at completed jobs within +/- 30% distance range
  - Matches cargo size category
  - Uses the last 90 days of data
  - Requires at least 3 similar jobs for historical blending
  - Historical average is weighted 40%, recommended price 60%

  ## Important Notes
  - Falls back gracefully to non-historical scoring when insufficient data
  - All new parameters have defaults for backwards compatibility
*/
