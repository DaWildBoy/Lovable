...
import { Bike, ChevronDown, LogOut, Loader2, ArrowLeft } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { useAuth } from '../contexts/AuthContext';
import { COURIER_TERMS_VERSION, courierSections } from '../components/CourierTermsOfServiceModal';
...
export function CourierTermsAcceptancePage({ onAccepted, onBack }: Props) {
  ...
  const handleAccept = async () => {
    ...
    try {
      const { error: updateError } = await supabase
        .from('profiles')
        .update({
          courier_terms_accepted_at: new Date().toISOString(),
          courier_terms_version: `courier-${COURIER_TERMS_VERSION}`,
        })
        .eq('id', user.id);
        ...
        <div className="w-full max-w-3xl animate-fade-in-up">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-gray-500">
              Please review and accept the driver terms to start accepting jobs on the platform
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <div className="px-5 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
              <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                MoveMeTT Ltd. Driver Agreement
              </span>
              <span className="text-xs text-gray-400">Version {COURIER_TERMS_VERSION}</span>
            </div>
          ...
          <div className="mt-5 bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
            <div className="flex items-start gap-3 mb-4">
              <input
                type="checkbox"
                id="courier-terms-checkbox"
                checked={checked}
                onChange={(e) => setChecked(e.target.checked)}
                className="mt-1 h-5 w-5 rounded border-gray-300 text-amber-600 focus:ring-amber-500 cursor-pointer flex-shrink-0"
              />
              <label htmlFor="courier-terms-checkbox" className="text-sm text-gray-700 leading-relaxed cursor-pointer">
                I have read, understood, and agree to the{' '}
                <span className="font-semibold text-gray-900">MoveMeTT Courier Terms of Service</span>.{' '}
                I acknowledge that I am an independent contractor and am bound by these terms for all jobs accepted through the Platform.
              </label>
            </div>
